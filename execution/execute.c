...
void handle_heredoc(char *delimiter, char *tmp_file, int remove_tabs)
{
    char *line;
...
void concatenate_files(char *final_file, t_list *her_docs)
{
    ...
    while (current)
    {
        ...
        handle_heredoc((char *)current->content, tmp_file, 0);  // Change 0 to 1 if you need to remove tabs
...
static void handle_redirections(t_cmd *cmd)
{
    ...
    if (cmd->her_docs)
    {
        ...
        concatenate_files(final_file, cmd->her_docs);
